...
class GenerateApiFunc:
    """API生成器"""
    def __init__(self, filename: str, prefix: str, name: str, class_name: str, output_dir: str) -> None:
        ...
        self.output_dir: str = output_dir  # 输出路径 "../ctp/api/generator/" or "../tts/api/generator/"
    ...
    def run(self) -> None:
        ...
        print(f"{self.output_dir}{self.prefix} {self.name} API生成成功")
    ...
    def generate_header_define(self) -> None:
        ...
        返回值:
            None: 无返回值，直接写入文件
        """
        filename = f"{self.output_dir}{self.prefix}_{self.name}_header_define.h"
    ...
    def generate_header_process(self) -> None:
        """"""
        filename = f"{self.output_dir}{self.prefix}_{self.name}_header_process.h"
        with open(filename, "w") as f:
    ...
    def generate_header_on(self) -> None:
        ...
        """
        filename = f"{self.output_dir}{self.prefix}_{self.name}_header_on.h"
    ...
    def generate_header_function(self) -> None:
        ...
        """
        filename = f"{self.output_dir}{self.prefix}_{self.name}_header_function.h"
        try:
            with open(filename, "w") as f:
    ...
    def generate_source_task(self) -> None:
        """"""
        filename = f"{self.output_dir}{self.prefix}_{self.name}_source_task.cpp"
        with open(filename, "w") as f:
            for name, d in self.callbacks.items():
    ...
    def generate_source_switch(self) -> None:
        """"""
        filename = f"{self.output_dir}{self.prefix}_{self.name}_source_switch.cpp"
    ...
    def generate_source_process(self) -> None:
        """"""
        filename = f"{self.output_dir}{self.prefix}_{self.name}_source_process.cpp"
        lines = []
    ...
    def generate_source_function(self) -> None:
        """"""
        filename = f"{self.output_dir}{self.prefix}_{self.name}_source_function.cpp"
    ...
    def generate_source_on(self) -> None:
        """"""
        filename = f"{self.output_dir}{self.prefix}_{self.name}_source_on.cpp"
    ...
    def generate_source_module(self) -> None:
        """"""
        filename = f"{self.output_dir}{self.prefix}_{self.name}_source_module.cpp"
...
if __name__ == "__main__":
    ...
    td_generator = GenerateApiFunc(
        "../ctp/api/include/ThostFtdcTraderApi.h",
        "ctp",
        "td",
        "TdApi",
        "../ctp/api/generator/"
