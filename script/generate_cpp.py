...
from script.generate_helper import create_output_dir


class GenerateCpp:
    """MD和TD CPP文件拼装器"""
    
    def __init__(self, filename: str, prefix: str, name: str, source_dir: str):
        ...
        self.source_dir: str = source_dir  # ../ctp/api/generator/
        ...
        # 定义所需的文件
        self.header_files = {
            'define': f'{self.source_dir}{self.prefix}_{self.name}_header_define.h',
            'function': f'{self.source_dir}{self.prefix}_{self.name}_header_function.h',
            'on': f'{self.source_dir}{self.prefix}_{self.name}_header_on.h',
            'process': f'{self.source_dir}{self.prefix}_{self.name}_header_process.h'
        }

        self.source_files = {
            'task': f'{self.source_dir}{self.prefix}_{self.name}_source_task.cpp',
            'switch': f'{self.source_dir}{self.prefix}_{self.name}_source_switch.cpp',
            'process': f'{self.source_dir}{self.prefix}_{self.name}_source_process.cpp',
            'function': f'{self.source_dir}{self.prefix}_{self.name}_source_function.cpp',
            'on': f'{self.source_dir}{self.prefix}_{self.name}_source_on.cpp',
            'module': f'{self.source_dir}{self.prefix}_{self.name}_source_module.cpp'
        }
        # md cpp 头文件内容
