...
@FileName   : generate_data_type.py
...
class DataTypeGenerator:
    ...
    def __init__(self, filename: str, prefix: str, output_dir: str) -> None:
    ...
    def run(self) -> None:
        ...
        parent_path: Path = current_file.parent.parent / "ctp/api"

        self.f_cpp = open(self.filename)
        self.f_define = open(f"{str(parent_path)}/{self.prefix}_constant.py", "w")
        self.f_typedef = open(f"{self.output_dir}{self.prefix}_typedef.py", "w")
...
if __name__ == "__main__":
    generator = DataTypeGenerator(
        "../ctp/api/include/ThostFtdcUserApiDataType.h",
        "ctp",
        "../ctp/api/generator/"
    )
    generator.run()
